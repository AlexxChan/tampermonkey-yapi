import { QueryStringArrayFormatEnum, RequestConfig, RequestFunctionParams } from './type.js'
...
const queryStringify = (
  key: string,
  value: any,
  arrayFormat: QueryStringArrayFormatEnum
): string => {
  let str = ''
  if (value !== null) {
    if (!Array.isArray(value)) {
      str = `${encodeURIComponent(key)}=${encodeURIComponent(value)}`
    } else if (arrayFormat === QueryStringArrayFormatEnum.indices) {
      str = value
        .map((v, i) => `${encodeURIComponent(`${key}[${i}]`)}=${encodeURIComponent(v)}`)
        .join('&')
    } else if (arrayFormat === QueryStringArrayFormatEnum.repeat) {
      str = value.map((v) => `${encodeURIComponent(key)}=${encodeURIComponent(v)}`).join('&')
    } else if (arrayFormat === QueryStringArrayFormatEnum.comma) {
      str = `${encodeURIComponent(key)}=${encodeURIComponent(value.join(','))}`
    } else if (arrayFormat === QueryStringArrayFormatEnum.json) {
...
export function prepare(requestConfig: RequestConfig, requestData: any): RequestFunctionParams {
  ...
  const getFormData = () => {
    ...
    const useNodeFormData =
      ...
      // @ts-ignore
      typeof global === 'object' && // @ts-ignore
      typeof global.process === 'object' && // @ts-ignore
      typeof global.process.versions === 'object' && // @ts-ignore
      global.process.versions.node !== null // @ts-ignore
